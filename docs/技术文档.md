...
- **Admin (管理后台):**
  - `GET /api/domains`: 获取域名列表 (支持分页、筛选、排序，返回完整信息)
  - `POST /api/domains`: 添加新域名
  - `GET /api/domains/{id}`: 获取单个域名完整详情
...
- `GET /api/domains/{domainId}/costs`: 获取某域名的成本列表
- `POST /api/domains/{domainId}/costs`: 为某域名添加成本记录
- `PUT /api/domains/{domainId}/costs/{costId}`: 更新成本记录
- `DELETE /api/domains/{domainId}/costs/{costId}`: 删除成本记录

**Landing Page API (主要为 Admin 配置，Public 通过 Domain/Portfolio API 获取渲染数据):**

- **Admin (管理后台):**
  - `GET /api/domains/{domainId}/landing_page`: 获取着陆页配置
  - `PUT /api/domains/{domainId}/landing_page`: 更新着陆页配置
...
- **Admin (管理后台):**
  - `GET /api/portfolios`
  - `POST /api/portfolios`
  - `GET /api/portfolios/{id}`: 获取米表完整配置详情
  - `PUT /api/portfolios/{id}`: 更新米表配置
  - `DELETE /api/portfolios/{id}`: 删除米表
  - `GET /api/portfolios/default`: 获取默认米表配置
  - `POST /api/portfolios/{id}/domains`: 向自定义米表添加域名关联
  - `DELETE /api/portfolios/{id}/domains/{domainId}`: 从自定义米表移除域名关联
  - `PUT /api/portfolios/{id}/domains/{domainId}`: 更新米表中特定域名的显示配置 (如价格覆盖)
- **Public (门户):**
...
- **Admin (管理后台):**
  - `GET /api/inquiries`: 获取询盘列表 (支持筛选、排序)
  - `GET /api/inquiries/{id}`: 获取单个询盘详情
  - `PUT /api/inquiries/{id}`: 更新询盘状态 (如已读、已回复)
- **Public (门户):**
  - `POST /api/public/inquire`: (公开接口, 无需认证但需安全措施) 访客提交询盘

**Watched Domain API (仅限 Admin - 管理后台):**

- `GET /api/watched_domains`
- `POST /api/watched_domains`
- `DELETE /api/watched_domains/{id}`
- `POST /api/watched_domains/{id}/check_status`: 手动触发检查状态
...
- `GET /api/ai/prompts`: 获取所有场景的提示词配置
- `GET /api/ai/prompts/{scene_code}`: 获取特定场景的提示词
- `PUT /api/ai/prompts/{scene_code}`: 更新特定场景的用户自定义提示词

**System Settings API (仅限 Admin - 管理后台):**

- `GET /api/settings`: 获取所有可配置的系统设置
